...
PowerTag::PowerTag() : DoubleOpTag(TString(L"power"))
{
}
...
TagCheck PowerTag::ProcessTag(TString & bounds, VariableContainer & globalVariables, TInterpretor & inter, IntLanguage& lang, TDataArray<BNFTag*>& tags)
{
	...
	if (tryRightDouble)
	{
		if (tryLeftDouble)
		{
			...
			de.left.returnValue->value.primFloat = pow(leftFloat,rightFloat);
		}
		else
		{
			de.left.returnValue->hold = AG_I_DOUBLE;
			de.left.returnValue->value.primFloat = pow(leftInt, rightFloat);
		}
	}
	else
	{
		...
		{
			de.left.returnValue->hold = AG_I_DOUBLE;
			de.left.returnValue->value.primFloat = pow(leftFloat, rightInt);
		}
		else
		{
			de.left.returnValue->hold = AG_I_DOUBLE;
