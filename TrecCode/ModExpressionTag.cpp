#include "stdafx.h"
#include "ModExpressionTag.h"


ModExpressionTag::ModExpressionTag() : DoubleOpTag(TString(L"mod_exp"))
{
}


ModExpressionTag::~ModExpressionTag()
{
}

TagCheck ModExpressionTag::ProcessTag(TString & bounds, VariableContainer & globalVariables, TInterpretor & inter, IntLanguage& lang, TDataArray<BNFTag*>& tags)
{
	...
	if (tryRightDouble)
	{
		if (tryLeftDouble)
		{
			...
			de.left.returnValue->value.primInt = (long long)(leftFloat) % (long long)rightFloat;
		}
		else
		{
			de.left.returnValue->hold = AG_I_S_INT;
			de.left.returnValue->value.primInt = leftInt % (long long)rightFloat;
		}
	}
	else
	{
